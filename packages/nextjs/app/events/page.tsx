...
const Events: NextPage = () => {
  ...
  const [fromBlockWindow, setFromBlockWindow] = useState<bigint>(0n);

  // Respect RPC max block range (Lisk Sepolia: 100_000)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const latest = await publicClient.getBlockNumber({ cacheTime: 0 });
        const window = 95_000n; // keep a safe margin below 100k
        const computed = latest > window ? latest - window : 0n;
        if (!cancelled) setFromBlockWindow(computed);
      } catch {
        // leave default 0n on failure
      }
    })();
    return () => {
      cancelled = true;
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="table table-zebra">
                ...
                <tbody>
                  {currentEvents.slice(0, 20).map((event: any, index: number) => (
